This additional function was for OpenLP and it was described in http://forums.openlp.org/discussion/2205/chords-websockets-and-more-i-want-to-help

Import chords_tags.conf as OpenLP configuration or add formating tags:
{cl}: <span class="chordline"> .. </span>
{c}: <span class="chord" style="display:n?ne;"> .. </span>

and unzip this file to OpenLP or manualy copy slide.html and slide.js to plugins\remotes\html\

Tomas